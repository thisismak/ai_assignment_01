...
# 安裝指南
1. 進入終端機的 bash 模式以執行命令
2. 使用 pip 安裝 playwright（網頁爬取）、pillow（圖片處理）和 requests（HTTP 請求）庫
- pip install playwright pillow requests
3. 安裝 Playwright 的瀏覽器依賴（如 Chromium），以支援網頁爬取
- playwright install
4. 執行名為 script.py 的 Python 腳本以運行程式
- python script.py
